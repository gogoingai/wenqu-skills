...
- 内容硬伤：translate 删除误判「是因为」翻译腔规则、步骤统一三步；review「反向验收」统一为「完成条件核对」、DM 补入 R1 豁免表、humanizer-zh.md 翻译腔润色；image 风格库「四选一」改「见下表」、style 选择依据集中到 SKILL.md、flow.md「踩坑14」改「踩坑5」、ref 字段统一 GitHub raw URL；write changelog 归档统一按天、删除 3 个老样例文件、.gitignore 逻辑修正。
- plugin description 统一为 A 版；package.json files 修正 `README.zh-CN.md` -> `README.en.md`。
- 插件包与 6 个单技能版本更新为 0.1.15。

## 0.1.14 - 写作素材安全边界与 Codex 图片比例支持

- `wenqu-write` 移除不再维护的 Claude Memory 完整文章样例；为画像快照、外部素材与后续会话加载补充敏感数据防护、提示注入防护与按需读取约束。
- `wenqu-image` 的 Codex 后端将 `--ar` 传给内置渲染脚本，向生图工具传递比例与建议尺寸；文档明确该后端为 best-effort，不能保证精确像素尺寸。
...
- 新增全局非敏感图片配置、仅本机的 `.env` 密钥文件与文章级 `config/image.json` 覆盖；支持 provider、模型、画幅与可选 API Base URL，参考图能力校验，以及 JSON 状态输出与安全重试。
- 更新 Agent 原生问答规则与逐张确认工作流、provider/model 溯源记录，并补充中英文 README 与离线 CLI/adapter 回归测试。
- README 默认改中文，英文版移至 `README.en.md`。
...
- WorkBuddy 直接复用 `.claude-plugin/marketplace.json`，不再维护重复的专用市场清单；并随仓库根 `VERSION`、Claude Code 插件一并统一为 `0.1.11`。
- 六个技能补充 WorkBuddy / SkillHub 元数据与 OpenClaw 主页链接；描述改为中文，保留英文触发词，并将适用范围扩展为通用中文内容创作。
...
- 外部工具资料改为独立模块目录：`open-websearch/` 完整记录十个搜索引擎、CLI 参数、JSON 协议与安装边界；`crawl4ai/` 记录 `crwl` 命令用法、配置、下载策略、按引擎的浏览器检索恢复，以及公众号 `src=11`（MicroMessenger UA、搜狗 Referer、`#js_content` 等待）抓取路径。
...
- 单篇素材库从 `references/materials.md` 单文件改为 `references/materials/` 目录：按内容类型分目录（`local/`、`articles/`、`papers/`、`docs/`），`index.md` 统一索引（来源 URL 必填、用途两类、标签检索）
...
- `wenqu-write` 增加内容来源与评测叙事、指令优先级、修改授权分级、文章状态和完成条件核对；骨架可检查正文偏离与章节完成条件
...
继续从文章 preferences.md 和专项审查报告归档：

- **article-write `SKILL.md` 修改流程加 1 条**：措辞打磨时，用户说某处“不好”“怪”，先讨论问题所在并给出候选表述，待用户明确确认后再改正文，不直接替换
- **article-write `style-guide.md` 节奏控制加 1 条**：正文不能只靠连续段落堆叙述，适当用加粗承担结构提示作用（突出关键结论、设计取舍、图表后的引导句）
...
基于《AI记忆系统与梦境》文章一次长会话的经验归档：大量时间花在措辞打磨，提炼出通用的「直白优先」规则和交互规范。特定偏好（如「查询->用户问题」）不升级到通用，留在文章自己的 `preferences.md`。

- **新增仓库根 `DEVELOPMENT.md`**：技能开发原则——经验归档的通用 vs 特定判断标准、归档流程、各 skill 文件分工（按经验类型对号入座）、改完同步（`git push`、`npx skills update`）
...
用户发现两个问题：① `.article-skills/{文件名}/` 按文件名分目录存储，文章一旦被用户重命名，存储目录和文章的映射就断了，下次加载会误判成"从没写过"；② 技能里很多"给用户看一眼再确认"的环节（画完图、连续小修改遇到待定项）只是输出文本，没有真正走结构化确认，容易被用户划水略过。另外，写完的草稿一直带着画图提示代码块、项目内部标识等创作期标记，没有一个"生成干净发布版"的收尾步骤。
...
- **article-write 新增版本记录**：`references/planning/changelog.md` 定义"草稿、事件、发布"三类版本表，写入每篇文章的 `references/changelog.md`；Step 4 首次定稿记 v1，"修改"流程每轮改完记 vN+1，语言审查小修改不单独占版本号
...
- **新增 `article-publish` 技能**：清洗草稿正文（去掉画图提示代码块、项目生成标识），生成标题候选、约100字简介、封面图（复用 article-image 单独定义的封面画图规范），产出到 `.article-skills/{文件名}/publish/v{N}/`，同时预留 `references/auto-publish.md` 作为未来接自动发布平台的扩展点，当前只写本地文件不做任何网络推送
...
- **未决风险要求现在问清楚，不能拖到执行阶段自己拍板**：比如素材不完整、deadline 逼近时怎么办，能现在问清楚的现在问，不要留到真遇到时才发现只能自己做主
...
原先本篇 context/skeleton 存在 `$HOME/.gogoingai/article-write/{项目目录名}/`（项目目录名 = `basename $(pwd)`）。这个方案在一个仓库里装着多篇互不相关文章时（比如一个笔记仓库存了几十篇不同主题的稿子）会出问题：所有文章共用同一个"项目目录名"分目录，context/skeleton 互相覆盖串味，写作历史区分不开。

- 存储位置迁移到**项目根目录**下的 `.article-write/` 子目录（`git rev-parse --show-toplevel`，非 git 仓库则用当前工作目录），类似 Claude Code 把 `.claude/` 放项目根目录的做法
- 分目录粒度从"项目目录名"改成"**文章文件名**"：`{项目根目录}/.article-write/{文件名}.context.md` / `.skeleton.md`，同一项目下的不同文章不再互相覆盖
...
用户展示了一套英文技术教程配图（llama.cpp 本地部署教程的四步流程图、笔记本插画构图、GGUF 模型名拆解图），确认喜欢该风格后翻译成中文逐张验证，收入第七套风格库：

- **新增 `pencil-sketch` 风格**：4 张参考图（横向四步编号流程、长条拆解、编号卡片、竖向判断分支流程、竖向三层架构），文档见 `references/styles/pencil-sketch.md`
- 核心特征：奶油纸感背景、彩色铅笔与蜡笔晕染图标（边缘带斜线排线阴影）、圆形编号徽章、虚线分隔、手写标题与同色系下划线、可爱吉祥物点缀。与已有的奶油描边风格（黑色粗描边、线条工整无阴影）、水彩涂鸦风格（无排线阴影、有装饰性星星火花）能明确区分开。
...
- 过程中先后踩了两个坑：①第一版验证图 prompt 把参考图里的每个视觉细节都写成文字描述，导致生成结果和参考图风格明显偏离（`--ref` 权重被文字描述稀释）；改成"参考图风格 + 内容"这种极简 prompt 后风格立刻复现。②标题字号容易被模型放大到占满画面，需要在 prompt 里显式强调"字号适中偏小、单行居中"
...
- 过程中定位到两个可复现的生图踩坑，写进该风格文档的"踩坑记录"一节：①箭头容易同图混用波浪、直线两种画法、且颜色乱跳，需要 prompt 里显式要求"全图箭头统一风格、颜色统一为一到两种色系"；②判断分支的"是、否"文字标签在专门修复箭头问题的那一轮容易被模型顺带删掉，需要同一句 prompt 里把"箭头风格统一"和"是否标签必须保留"两个要求都写全，只强调一个会导致另一个跑丢
- `SKILL.md` 风格库表格、依赖清单同步加入这套风格的入口

## 0.0.8 - 骨架自检新增章节顺序依赖检查

`article-write` Step 3.4 骨架质量自检加一项：检查每节用到的概念、术语是否都已在前面章节出现过。在骨架阶段发现顺序问题成本很低（调整章节顺序即可），拖到正文阶段才发现，要么破坏行文插入解释，要么回头改前面章节，成本高得多。这条是独立判断"对写文章这件事本身有没有用"后加的，不是照抄参考工具的清单条目。
...
- 其余按关注点拆分：`core-principles.md`（核心原则、禁用泳道图、节点标签规则）、`pitfalls.md`（常见踩坑）、`design-principles.md`（设计四大原则与配色）、`diagram-type-selector.md`（图类型选型指南）
...
**同一版本里顺带解决的另一个外部依赖**：`article-image` 之前依赖单独安装的 `gpt-image-2` skill 才能真正生成图片。`gen.sh`/`extract_image.py` 本身很小、无隐藏依赖，且 gpt-image-2 声明 MIT 协议，直接复制进 `article-image/scripts/`（新增 `gpt-image-2-gen.sh`、`extract_image.py`、`THIRD_PARTY_NOTICES.md`），不再需要单独安装该 skill。唯一剩下的外部依赖是 `codex` CLI 本身——这是真正意义上"没法吸收"的第三方工具，不是可以复制内容进来的技能。
...
- 全部改成 `~/.agents/skills/<name>/...`——这是 `npx skills` 每次安装都保证会建的标准位置，不依赖具体装了哪个 agent
