...
- `--limit N` 是**所有选中引擎共享的总上限**，CLI 会平均分摊给各引擎；若三个引擎想各取约 3 条，应设 `--limit 9`，不要误设为 3。
...
- `partialFailures` 的每项有 `engine`、`code`、`message`。单引擎失败时保留其他引擎结果，并把失败写入本轮记录。
...
2. `partialFailures` 列出的引擎：记录 `code` 与 `message`，不阻塞其他候选。
3. 没有结果且没有失败记录的引擎：记为“本次无候选”，不据此判断其能力，也不阻塞收集。

stdout 只解析 JSON；stderr 作为执行日志保留。素材台账只写实际进入候选池的来源，执行记录另存失败与无候选信息。

## 运行模型与边界

一次性 `search` 会尝试复用已存在的本地 daemon，未找到时可直接执行；Wenqu **不启动 daemon**、不传 `--spawn`、不写 daemon 与 MCP 配置。`--daemon-url` 会强制指定 daemon 且禁用直接执行，不适合文库默认路径。
