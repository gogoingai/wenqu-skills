...
`crwl` 处理的是已确认 URL 的下载，不承担全网搜索。先按对应站点食谱抓取；出现验证页、空正文、登录、付费或访问限制时，记录原因并改用 agent 原生下载或公开替代来源，不反复请求，也不绕过访问控制。
...
只在 agent 已从原生搜索或其它候选中确认**文章标题**（必要时还有公众号名）后使用本路径。查询词应包含该标题；不要先把搜狗微信结果链接保存下来、再另开 `crwl` 访问，更不能默认点击结果页第一条。
...
| 现象 | 处理 |
| --- | --- |
| 环境异常、验证页、正文极短 | 按公众号 UA/Referer 食谱重试一次，仍失败原生下载并记录反爬 |
| 浏览器可执行文件不存在 | agent 运行 setup 与 doctor；仍失败原生下载 |
| 超时、乱码或脚本残留 | 一次 `markdown-fit` 或合理等待重试；仍失败原生下载 |
| 登录、付费或访问控制 | 记录边界，寻找公开替代来源；不绕过 |
