---
name: wenqu-library
description: 文章创作前的素材积累。按"规划、搜索、下载、整理"四步收集素材：联网搜索候选来源，用 crwl 抓取网页（含微信公众号），整理成带索引的本篇素材目录与跨文章可复用的素材库。触发关键词：收集素材、整理资料、建素材库、沉淀知识、积累案例、抓取网页、收集网页素材。
---
...
```markdown
| 编号 | 摘要 | 来源类型 | 来源 | 文件路径 | 用途 | 标签 | 关联章节 | 日期 |
| M1 | xxx 机制源码摘录 | 实现事实 | src/cache.py:42 | local/cache-lru.md | 事实素材 | 缓存,LRU | 2.1 | 2026-07-25 |
| M2 | 某公众号文章：XX 系统实践 | 外部研究 | https://mp.weixin.qq.com/s/xxx | articles/mp.weixin.qq.com/xx.md | 写法参考 | 架构 | 待定 | 2026-07-25 |
```
...
4. **文件路径相对 materials/ 填写**；只登记不落文件的短素材（几行数字、一句话事实）该列填 `-`
5. **用途二分**：`写法参考`（相似文章，不能当事实来源）/ `事实素材`（支撑正文的机制、数字、案例）
6. **标签**：2~4 个关键词，方便 grep 检索；素材多了以后按标签/目录检索，不靠通读
7. **关联章节**：收集阶段先填「待定」，骨架定稿后回填章节号
8. 素材有实体文件时才进目录；小片段直接写在 index.md 摘要列即可，不为每条都建文件
...
每一步的执行细则 → 查 `references/collection-playbook.md`；crwl 命令参数与实测产物结构 → 查 `references/crwl-cookbook.md`。

### 1. 规划

根据写作规划产出**收集清单**：要找哪些相似文章（写法参考）、哪些相关素材（事实素材）、中英文搜索关键词组合、预计抓取数量。清单列给用户确认后开始执行（**全流程唯一的确认点**）。

### 2. 搜索

用 agent 自带的联网搜索工具按关键词逐组搜索，产出候选 URL 清单。筛掉明显不相关和低质量来源、同内容转载只留原始出处后**直接进入下载，不再让用户逐条确认**。用户直接提供 URL 时跳过本步。

### 3. 下载

优先用 `crwl`（Crawl4AI CLI）抓取，产物直接落到本篇素材目录对应分类下：

- 未安装 crwl 时**主动提议帮用户安装**（`uv tool install crawl4ai` + `crawl4ai-setup`），用户拒绝或安装失败才降级为 agent 自带抓取
- 单页、整站 deep-crawl（`--max-pages` 必须显式限制）、微信公众号（UA 伪装 + 等待正文渲染）三类命令见 `references/crwl-cookbook.md`
...
8. **主动安装**：检测到 crwl 未安装时主动提议帮用户装，而不是直接降级打发了事
