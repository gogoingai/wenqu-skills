...
| 箭头 | 简单黑色直线箭头表示顺序，判断分支用"是/否"文字加对应颜色（绿/红）标注 |

完整风格选择见 `SKILL.md` 风格库表。

### 生成规则（--ref 与 prompt 缺一不可，且 prompt 要简短——参考 pitfalls.md 踩坑 9）
...
| 文件 | 图类型 | 特点 |
| --- | --- | --- |
| `pencil-sketch-01-stage-flow.png` | 横向四步编号流程 | 构建→下载→运行→验证，四色编号卡片，顶部小标题与下划线，底部星星吉祥物横幅总结一句话 |
| `pencil-sketch-02-breakdown.png` | 长条拆解与编号卡片 | 顶部长条框按分段配色显示完整字符串，向下箭头引到多个编号小卡片逐段解释，底部横幅公式化总结 |
