...
#!/bin/zsh
# 自动生成的 picgo wrapper，由 wenqu-image skill 创建
...
run_check() {
  ...
  step "检测生图脚本（vendored，随 wenqu-image 一起分发）"
  ...
  if [[ -n "$gen_sh" ]]; then
    ...
    echo ""
    echo "   这个文件应该和 gen-image.sh 一起装好，缺失说明 wenqu-image 技能安装不完整，"
...
run_generate() {
  ...
  local gen_sh; gen_sh=$(find_gen_sh)
  if [[ -z "$gen_sh" ]]; then
    err "找不到 $SCRIPT_DIR/gpt-image-2-gen.sh，wenqu-image 技能安装不完整"
    echo "请先运行：bash scripts/gen-image.sh --check"
    exit 3
  fi
