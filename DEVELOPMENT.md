...
标准顺序是：创建功能分支 -> 本地运行对应检查 -> 提交并创建 PR -> 查看 PR 中的 Skills Eval 评论与检查 -> 获得合并授权后合入 `master`。平台正式发布是合并后的独立动作，仍需明确授权并按 [RELEASE.md](RELEASE.md) 执行。
...
| 修改插件、市场 | `VERSION`、`.claude-plugin/plugin.json`、共享的 `.claude-plugin/marketplace.json` | `skills-eval check . --external-target claude-plugin --external-target workbuddy --external-target clawhub --external-target skillhub`（skillhub 需本机已登录；PR 检查会用 secret 自动补跑） |
